...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('makeup_punch_rules', (t) => {
  ...
  await knex.schema.createTable('makeup_punch_requests', (t) => {
  ...
  await knex.schema.raw(`
    CREATE INDEX idx_makeup_punch_requests_user ON makeup_punch_requests(user_id);
    CREATE INDEX idx_makeup_punch_requests_status ON makeup_punch_requests(status) WHERE status = 'pending';
  `);
}
